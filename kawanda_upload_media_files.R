...
flowerid2 <- readChar("flowerid_ka.txt", 10)
...
while(flower_id2 == ''){
  ...
  post.flower.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                  data_value='KawandaFlowering.csv',
                                  data_type='media',
                                  xform=446576,
                                  data_file=fileUpload(filename = "KawandaFlowering.csv",contentType = 'text/csv'),
...
cat(flower_id2, file = "flowerid_ka.txt")


# 2b. banana

bananaid2 <- readChar("bananaid_ka.txt", 10)
hdr=c(Authorization=paste("Temptoken ",TOKEN_KEY))
...
while(banana_id2 ==''){
  ...
  post.banana.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                  data_value='KawandaBananaData.csv',
                                  data_type='media',
                                  xform=446576,
                                  data_file=fileUpload(filename = "KawandaBananaData.csv",contentType = 'text/csv'),
...
cat(banana_id2, file = "bananaid_ka.txt")


# 2c.. Plantlets

plantletid2 = readChar("plantletid_ka.txt", 10)
hdr=c(Authorization=paste("Temptoken ",TOKEN_KEY))
...
while(plantlet_id2 ==''){
  ...
  post.plantlets.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                     data_value='KawandaPlantlets.csv',
                                     data_type='media',
                                     xform=446576,
                                     data_file=fileUpload(filename = "KawandaPlantlets.csv",contentType = 'text/csv'),
                                     .opts=list(httpheader=header), verbose = TRUE)
  # get ID
  plantlets.raw.result.json<-fromJSON(post.plantlets.results)
  plantlet_id2 <- plantlets.raw.result.json$id
}
cat(plantlet_id2, file = "plantletid_ka.txt")


# 2d. crosses per female plot

crossesid2 = readChar("crossesid_ka.txt", 10)
hdr=c(Authorization=paste("Temptoken ",TOKEN_KEY))
...
while(crosses_id2 ==''){
  ...
  post.ncrosses.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                    data_value='KawandaCrossesPerFemalePlot.csv',
                                    data_type='media',
                                    xform=446576,
                                    data_file=fileUpload(filename = "KawandaCrossesPerFemalePlot.csv",contentType = 'text/csv'),
                                    .opts=list(httpheader=header), verbose = TRUE)
  # get  ID
  ncrosses.raw.result.json<-fromJSON(post.ncrosses.results)
  crosses_id2 <- ncrosses.raw.result.json$id
}
cat(crosses_id2, file = "crossesid_ka.txt")

# 1d. status 

statusid1 = readChar("statusid_ka.txt", 10)
hdr=c(Authorization=paste("Temptoken ",TOKEN_KEY))
...
while(status_id1 ==''){
  ...
  post.nstatus.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                   data_value='KawandaPlantStatus.csv',
                                   data_type='media',xform=446576,
                                   data_file=fileUpload(filename = "KawandaPlantStatus.csv",contentType = 'text/csv'),
                                   .opts=list(httpheader=header), verbose = TRUE)
  # get  ID
  nstatus.raw.result.json<-fromJSON(post.nstatus.results)
  status_id1 <- nstatus.raw.result.json$id
}
cat(status_id1, file = "statusid_ka.txt")


# 1d. contamination 

contaminationid1 = readChar("contaminationid_ka.txt", 10)
hdr=c(Authorization=paste("Temptoken ",TOKEN_KEY))
...
while(contamination_id1 ==''){
  ...
  post.ncontamination.results <- postForm("https://api.ona.io/api/v1/metadata.json",
                                          data_value='KawandaContamination.csv',
                                          data_type='media',xform=446576,
                                          data_file=fileUpload(filename = "KawandaContamination.csv",contentType = 'text/csv'),
                                          .opts=list(httpheader=header), verbose = TRUE)
  # get  ID
  ncontamination.raw.result.json<-fromJSON(post.ncontamination.results)
  contamination_id1 <- ncontamination.raw.result.json$id
}
cat(contamination_id1, file = "contaminationid_ka.txt")
